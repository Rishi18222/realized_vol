"""Trade / hourly / daily / risk CSVs and charts (v2 schemas)."""
...
def write_result(spec_id: str, res: EngineResult, out_dir: Path | None = None) -> Path:
    ...
    schemas.conform(res.trades, schemas.TRADES).to_csv(d / "trades.csv", index=False)
    schemas.conform(res.hourly, schemas.HOURLY).to_csv(d / "hourly.csv", index=False)
    schemas.conform(res.daily, schemas.DAILY).to_csv(d / "daily.csv", index=False)
    schemas.conform(res.risk, schemas.RISK).to_csv(d / "risk.csv", index=False)
    schemas.conform(res.skips, schemas.SKIPS).to_csv(d / "skips.csv", index=False)
...
def _equity_chart(spec_id: str, res: EngineResult, path: Path) -> None:
    ...
    if res.trades.empty:
        ...
        title = C.BACKTESTS[spec_id].title if spec_id in C.BACKTESTS else spec_id
        ax.set_title(f"{spec_id}  {title}  hedge={C.HEDGE_FREQUENCY}")
...
def _vrp_chart(spec_id: str, res: EngineResult, path: Path) -> None:
    ...
    if res.trades.empty or "var_ratio" not in res.trades.columns:
        ax.set_title(f"{spec_id}: no variance-ratio scatter")
    else:
        t = res.trades.dropna(subset=["var_ratio", "pnl"])
        ax.scatter(t["var_ratio"], t["pnl"], c=np.where(t["pnl"] >= 0, "#2a7", "#c44"), alpha=0.75)
        ax.axhline(0, color="#666", lw=0.8)
        ax.axvline(1.0, color="#666", lw=0.8)
        ax.set_xlabel("Entry variance ratio (IV² / matched-tenor RV²)")
        ax.set_ylabel("Trade P&L (₹)")
        ax.set_title(f"{spec_id} var-ratio vs P&L")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(path, dpi=120)
    plt.close(fig)
...
def write_summary(results: dict[str, EngineResult], path: Path | None = None) -> pd.DataFrame:
    ...
    for k, res in results.items():
        ...
        rows.append(
            dict(
                ...
                pnl_hedge=float(t["pnl_hedge"].sum()) if "pnl_hedge" in t.columns else 0.0,
            )
        )
    ...
    if not df.empty:
        ...
        ax.set_title(f"Index/Stock VRP A–H  hedge={C.HEDGE_FREQUENCY}")
